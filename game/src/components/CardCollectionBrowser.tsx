...
interface CardReferenceCardProps {
  cardId: string;
...
export function CardReferenceCard({
  cardId,
  instance,
  count = 1,
  mode = "inspect",
    ...
    : card.kind === "status"
      ...
      : card.tags.includes("generated")
        ...
        : card.tags.includes("basic")
          ? "Basic"
          : "Team";
  const className = `collection-card${owner ? " has-owner" : ""}${rare ? " is-rare" : ""}${selected ? " is-selected" : ""}`;
  const style = {
  ...
  const content = (
    <>
      <span
        className={`collection-card__count${showCost ? " collection-card__count--cost" : ""}`}
        aria-label={showCost ? (unplayable ? "Unplayable" : `${card.cost} Focus`) : undefined}
      >
        {showCost ? (unplayable ? "—" : card.cost) : `×${count}`}
      </span>
      ...
      <span className="collection-card__tags">
        {rare && <span className="is-rare">Rare</span>}
        ...
        {unplayable && <span className="is-risk">Unplayable</span>}
      </span>
  ...
  return (
    <article
      ...
      aria-label={
        showCost
          ? `${card.name}. ${unplayable ? "Unplayable." : `Costs ${card.cost} Focus.`} ${card.rules}`
          : `${card.name}, ${count} ${count === 1 ? "copy" : "copies"}${instance?.generatedBy ? `, generated by ${instance.generatedBy.sourceCardId} on Day ${instance.generatedBy.day}` : ""}${instance?.exhausted ? `, exhausted on Day ${instance.exhausted.day}` : ""}`
      }
    >
      {content}
    </article>
